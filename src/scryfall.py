...
_SCRYFALL_ID_API = "https://api.scryfall.com/cards/{card_id}"
...
_RELATED_PRINTABLE_TYPES = ("token", "emblem", "dungeon", "card")
...
def fetch_related_tokens(card_ids: list[str]) -> list[DeckCard]:
    """Return tokens and token extras generated by the supplied Scryfall card IDs."""
    tokens: list[DeckCard] = []
    seen_names: set[str] = set()
    for card_id in dict.fromkeys(card_ids):
        if not card_id:
            continue
        try:
            response = _throttled_get(_SCRYFALL_ID_API.format(card_id=card_id))
            response.raise_for_status()
            parts = response.json().get("all_parts", [])
        except requests.RequestException as exc:
            _log.warning(
                "No se pudieron consultar las fichas de Scryfall para %s: %s", card_id, exc
            )
            continue
        for part in parts:
            if not isinstance(part, dict) or str(part.get("id", "")) == card_id:
                continue
            if not str(part.get("type_line", "")).casefold().startswith(_RELATED_PRINTABLE_TYPES):
                continue
            token_id = str(part.get("id", ""))
            name = str(part.get("name", ""))
            name_key = name.casefold()
            if name and token_id and name_key not in seen_names:
                seen_names.add(name_key)
                tokens.append(DeckCard(name, "", "", 1, "token", token_id))
    return tokens
...
def fetch_card_by_id(card_id: str) -> ScryfallCard:
    """Return front/back image URLs for a Scryfall card UUID."""
    try:
        resp = _throttled_get(_SCRYFALL_ID_API.format(card_id=card_id))
        resp.raise_for_status()
        data = resp.json()
    except requests.HTTPError as exc:
        raise ScryfallError(f"Carta no encontrada en Scryfall: {card_id} ({exc})") from exc
    except requests.RequestException as exc:
        raise ScryfallError(f"Error de conexión con Scryfall ({card_id}): {exc}") from exc
    if "image_uris" in data:
        return ScryfallCard(data["image_uris"]["large"], None)
    ...
    raise ScryfallError(f"Formato de respuesta inesperado de Scryfall para {card_id}")
...
def download_card_images(card: DeckCard, dest_dir: Path) -> tuple[Path, Path | None]:
    ...
    if card.scryfall_id:
        scryfall = fetch_card_by_id(card.scryfall_id)
        slug = card.scryfall_id
    else:
